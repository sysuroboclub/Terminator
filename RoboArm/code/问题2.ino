...
#define l1 0.078f
#define l2 0.055f
float Target1=100,Target2=90,Target3=82,Target4=86,Target5=0,Target6=90;  //6个舵机的目标值
...
float TargetX=0.105,TargetY=0.133,Target_Beta=0,Target_Alpha=0,Target_Gamma=0;               //电机目标值
char Flag_Way;  //控制方式 坐标控制还是单独控制
float Speed=5;  //控制舵机运动的速度
...
void my_control(float Step){
  ...
  Target_Gamma=0;*/
  if(control_number==0){
    if(TargetY>-0.05) TargetY-=Step;
    else if(Target5<80) Target5+=Step*3000;
    else control_number=1;
    }
  if(control_number==1){
    if(TargetY<0.045) TargetY+=Step;
    else control_number=2;
  }
  if(control_number==2){
    if(Target_Beta>-20) Target_Beta-=Step*3000;
    else if(TargetY>-0.015) TargetY-=Step*2;
    else if(Target5>0) Target5-=Step*3000;
